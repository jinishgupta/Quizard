...
// Hint cooldown periods (seconds) — must match backend
const HINT_COOLDOWNS = {
  eliminate: 15,
  clue: 20,
  firstLetter: 10,
};
...
export default function PlayScreen() {
  ...
  const { isActive: gamePassActive, formattedTime: passTimeLeft } = useGamePass();
  ...
  // Hint cooldown states
  const [eliminateCooldown, setEliminateCooldown] = useState(0);
  const [clueCooldown, setClueCooldown] = useState(0);
  const [firstLetterCooldown, setFirstLetterCooldown] = useState(0);
  ...
  useEffect(() => {
    cooldownTimerRef.current = setInterval(() => {
      setEliminateCooldown(prev => Math.max(0, prev - 1));
      setClueCooldown(prev => Math.max(0, prev - 1));
      setFirstLetterCooldown(prev => Math.max(0, prev - 1));
    }, 1000);
    ...
    createSession.mutate(
      { categoryId, difficulty: 'medium' },
      {
        onSuccess: (data) => {
          ...
          setTimeLeft(data.session.question?.timeLimit || 30);
        },
        onError: (error) => {
  ...
  // Build the questions array from session data
  // The backend returns one question at a time, we accumulate them
  const question = session?.question || session?.questions?.[currentQ];
  ...
  const timerPercent = question ? (timeLeft / (question.timeLimit || 30)) * 100 : 0;
  ...
  const handleTimeUp = useCallback(() => {
    if (answerState === 'unanswered' && question) {
      ...
      submitAnswer.mutate({
        ...
        answer: '__TIME_UP__',
        timeSpent: question.timeLimit || 30,
      });
      setResults(prev => [
        ...prev,
        { questionIndex: currentQ, selectedIndex: null, correct: false, timeSpent: question.timeLimit || 30 },
      ]);
    }
  ...
  const handleAnswer = (index) => {
    if (isAnswered || !question) return;
    clearInterval(timerRef.current);
    const timeSpent = (question.timeLimit || 30) - timeLeft;
    ...
    submitAnswer.mutate(
      ...
      {
        onSuccess: (data) => {
          const correct = data.result?.isCorrect ?? false;
          setAnswerState(correct ? 'correct' : 'wrong');
          setResults(prev => [...prev, { questionIndex: currentQ, selectedIndex: index, correct, timeSpent }]);
        },
        onError: () => {
          // Fallback: if API fails, still show result based on local data
  ...
  const handleNext = () => {
    ...
    if (nextQ >= totalQuestions) {
      completeSession.mutate(session.sessionId || session.id, {
        onSuccess: (data) => {
  ...
  // Hints are FREE with active game pass — but have cooldown timers
  const handleEliminate = () => {
    ...
    if (eliminateCooldown > 0) {
      toast.info(`Hint on cooldown. Wait ${eliminateCooldown}s`);
      return;
    }

    hintEliminate.mutate(
      { sessionId: session.sessionId || session.id, questionIndex: currentQ },
      {
        onSuccess: (data) => {
          setEliminatedOptions(data.eliminatedOptions || data.eliminatedIndices || []);
          setEliminateCooldown(data.cooldownSeconds || HINT_COOLDOWNS.eliminate);
  ...
  const handleClue = () => {
    ...
    if (clueCooldown > 0) {
      toast.info(`Hint on cooldown. Wait ${clueCooldown}s`);
      return;
    }
    ...
    hintClue.mutate(
      { sessionId: session.sessionId || session.id, questionIndex: currentQ },
      {
        onSuccess: (data) => {
          ...
          setClueCooldown(data.cooldownSeconds || HINT_COOLDOWNS.clue);
  ...
  const handleFirstLetterReveal = () => {
    ...
    if (firstLetterCooldown > 0) {
      toast.info(`Hint on cooldown. Wait ${firstLetterCooldown}s`);
      return;
    }
    ...
    hintFirstLetter.mutate(
      { sessionId: session.sessionId || session.id, questionIndex: currentQ },
      {
        onSuccess: (data) => {
          ...
          setFirstLetterCooldown(data.cooldownSeconds || HINT_COOLDOWNS.firstLetter);
  ...
  const getOptionClass = (index) => {
    ...
    const isEliminated = Array.isArray(eliminatedOptions) && (
      eliminatedOptions.includes(index) || eliminatedOptions.includes(question.options[index])
    );
    if (isEliminated) return 'answer-btn dimmed';
    if (!isAnswered) return 'answer-btn';
    const correctAnswer = submitAnswer.data?.result?.correctAnswer || question.correctAnswer;
              ...
              <div className="relative">
                <svg width="80" height="80" viewBox="0 0 80 80">
                  ...
                  <circle
                    ...
                    style={{ filter: timeLeft <= 5 ? 'drop-shadow(0 0 6px rgba(239,68,68,0.6))' : 'drop-shadow(0 0 4px rgba(249,115,22,0.4))' }}
                  />
                </svg>
                <div className="absolute inset-0 flex items-center justify-center">
                  <span className="text-xl font-black font-mono tabular-nums" style={{ color: timerColor }}>
                    {timeLeft}
                  </span>
                </div>
              </div>
              ...
              {question.options.map((option, index) => (
                <motion.button
                  ...
                  style={{
                    ...
                    opacity: isOptionDisabled(index) && !isAnswered ? 0.3 : 1,
                  }}
                ...
                >
                  <span
                    ...
                    style={{
                      background: isAnswered && index === correctIndex ? 'rgba(34,197,94,0.3)' : 'rgba(249,115,22,0.15)',
                      color: isAnswered && index === correctIndex ? '#22c55e' : '#f97316',
                    }}
                  >
                    {String.fromCharCode(65 + index)}
                  </span>
                  <span className="text-sm md:text-base font-semibold text-white leading-snug">{option}</span>
                  {isAnswered && index === correctIndex && <CheckCircle size={18} className="ml-auto text-green-300 shrink-0" />}
                  {isAnswered && index === selectedAnswer && index !== correctIndex && <XCircle size={18} className="ml-auto text-red-300 shrink-0" />}
                </motion.button>
              ))}
            </div>
                  ...
                  <div>
                    ...
                    {answerState !== 'correct' && (
                      <p className="text-sm text-gray-300">
                        Correct answer: <span className="font-bold text-white">{correctAnswer || question.correctAnswer}</span>
                      </p>
                    )}
                    {submitAnswer.data?.result?.explanation && (
      ...
      >
        {!isAnswered && (
          <div className="flex items-center justify-center gap-2 flex-wrap max-w-2xl mx-auto">
            <span className="text-xs text-gray-600 font-medium mr-1">Hints (free):</span>
            <button
              onClick={handleEliminate}
              disabled={eliminatedOptions.length > 0 || hintEliminate.isPending || eliminateCooldown > 0}
              className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-bold transition-all duration-150 ${
                eliminatedOptions.length > 0 || hintEliminate.isPending || eliminateCooldown > 0
            ...
            <button
              onClick={handleClue}
              disabled={!!clueText || hintClue.isPending || clueCooldown > 0}
              className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-bold transition-all duration-150 ${
                clueText || hintClue.isPending || clueCooldown > 0 ? 'opacity-40 cursor-not-allowed' : 'hover:scale-105 active:scale-95'
              }`}
              style={{
            ...
            <button
              ...
              disabled={!!firstLetter || hintFirstLetter.isPending || firstLetterCooldown > 0}
              className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-bold transition-all duration-150 ${
                firstLetter || hintFirstLetter.isPending || firstLetterCooldown > 0 ? 'opacity-40 cursor-not-allowed' : 'hover:scale-105 active:scale-95'
              }`}
              style={{
